...
pub enum AsyncResult {
    ...
    StakeLoaded {
        wallet_idx: usize,
        stakes: Result<Vec<WalletStakeInfo>>,
    },
    SubnetsLoaded(Result<Vec<DynamicSubnetInfo>>),
    MetagraphLoaded {
        netuid: u16,
        neurons: Result<Vec<NeuronDisplay>>,
    },
    Message(String),
    Error(String),
}
...
impl App {
    ...
    pub fn selected_wallet_balance(&self) -> Option<f64> {
        self.selected_wallet
            .and_then(|i| self.wallet_balances.get(i).copied().flatten())
    }
    ...
    async fn auto_connect(&mut self) {
        ...
        if self.wallets.is_empty() {
            self.add_message(
                "No wallets found. Create one with 'lt wallet create <name>'".to_string(),
            )
            .await;
            self.add_message("Press 'c' to try connecting after creating a wallet.".to_string())
                .await;
            return;
        }
        ...
        self.add_message(format!("⏳ Connecting to {}...", network))
            .await;
        ...
        match key {
            ...
            KeyCode::End => {
                if !self.metagraph_neurons.is_empty() {
                    self.metagraph_table_state
                        .select(Some(self.metagraph_neurons.len() - 1));
                }
            }
            KeyCode::Char('r') | KeyCode::F(5) => {
                if !self.is_loading {
            ...
            KeyCode::Char('/') => {
                // TODO: Search
                self.add_message("Search not yet implemented".to_string())
                    .await;
            }
            _ => {}
        }
    ...
    async fn handle_async_result(&mut self, result: AsyncResult) {
        match result {
            ...
            AsyncResult::WalletLoaded(res) => match res {
                Ok(wallet) => {
                    self.add_message(format!("✓ Loaded wallet: {}", wallet.name))
                        .await;
                }
                Err(e) => {
                    self.add_message(format!("✗ Failed to load wallet: {}", e))
                        .await;
                }
            },
            AsyncResult::BalanceLoaded {
                wallet_idx,
                balance,
            } => {
                match balance {
            ...
            AsyncResult::SubnetsLoaded(res) => match res {
                Ok(subnets) => {
                    ...
                    self.add_message(format!("✓ Loaded {} subnets", count))
                        .await;
                }
                Err(e) => {
                    self.add_message(format!("✗ Failed to load subnets: {}", e))
                        .await;
                }
            },
            AsyncResult::MetagraphLoaded { netuid, neurons } => match neurons {
                Ok(n) => {
                    let count = n.len();
                    ...
                    self.add_message(format!("✓ Loaded {} neurons for subnet {}", count, netuid))
                        .await;
                }
                Err(e) => {
                    self.add_message(format!("✗ Failed to load metagraph: {}", e))
                        .await;
                }
            },
        }
    ...
    pub fn fetch_metagraph(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            let _ = tx
                .send(AsyncResult::MetagraphLoaded {
                    netuid,
                    neurons: result,
                })
                .await;
    ...
    pub fn fetch_balance(&mut self, wallet_idx: usize) {
        ...
        tokio::spawn(async move {
            ...
            let _ = tx
                .send(AsyncResult::BalanceLoaded {
                    wallet_idx,
                    balance: result,
                })
                .await;
    ...
    pub fn fetch_all_balances(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            let _ = tx
                .send(AsyncResult::Message(format!("✓ Loaded {} balances", count)))
                .await;
        });
    }
...
async fn fetch_metagraph_data(ctx: &AppContext, netuid: u16) -> Result<Vec<NeuronDisplay>> {
    ...
    let neurons = discovery.get_all_neurons().map_err(|e| {
        crate::errors::Error::network(format!("Failed to discover neurons: {}", e))
    })?;

    let displays: Vec<NeuronDisplay> = neurons
        .into_iter()
        .map(|n| {
            NeuronDisplay {
                uid: n.uid,
...
async fn fetch_subnet_data(ctx: &AppContext) -> Result<Vec<DynamicSubnetInfo>> {
    ...
    let subnets = bittensor_rs::queries::get_all_dynamic_info(&client)
        .await
        .map_err(|e| {
            crate::errors::Error::network(format!("Failed to get dynamic info: {}", e))
        })?;
...
async fn fetch_wallet_balance(ctx: &AppContext, wallet: &WalletInfo) -> Result<f64> {
    ...
    let balance = bittensor_rs::queries::get_balance(&client, address)
        .await
...
async fn fetch_wallet_stakes(
    ctx: &AppContext,
    wallet: &WalletInfo,
) -> Result<Vec<WalletStakeInfo>> {
    ...
    let stake_infos = bittensor_rs::queries::get_stake_info_for_coldkey(&client, address)
        .await
        .map_err(|e| crate::errors::Error::network(format!("Failed to get stakes: {}", e)))?;

    let stakes: Vec<WalletStakeInfo> = stake_infos
        .into_iter()
        .map(|info| WalletStakeInfo {
            netuid: info.netuid,
